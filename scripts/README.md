...
| [`Test-DemoDebuggers.ps1`](Test-DemoDebuggers.ps1) | Proves the demo-breakpoint attach on this machine: VS Code, and on Windows Visual Studio 2026, each attach their debugger to a test process and let it go (the `DebuggerLiveTests` opt-in). Builds the Visual Studio helper when missing | Read-only apart from the helper build |
| [`Test-LinuxContainer.ps1`](Test-LinuxContainer.ps1) | Builds the demo and runs the deterministic suite inside the official .NET 10 SDK Linux container from this checkout, so a Windows or macOS presenter can prove the portable build and the "Visual Studio unavailable here" path before pushing | Read-only; nothing is written back to the checkout |
| [`Stop-CaesareaDemo.ps1`](Stop-CaesareaDemo.ps1) | The inverse, for when Ctrl+C is not available: asks the Aspire CLI to stop the AppHost, kills whatever survived (the host, its dcp orchestrators, the `dotnet run` wrappers, the services built here), and removes the CLI's stale socket files that would fail the next start | Yes — with nothing running it only prunes socket files |
